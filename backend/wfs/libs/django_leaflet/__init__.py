...
if DEFAULT_CENTER is not None and not (
    ...
    raise ImproperlyConfigured(
        "LEAFLET_CONFIG['DEFAULT_CENTER'] must be an list/tuple with two elements - (lon, lat)"
    )
        ...
        for resource_type in RESOURCE_TYPE_KEYS:
            ...
            for i, url in enumerate(urls):
                ...
                if url_parts.scheme or url_parts.path.startswith("/"):
                    ...
                    urls[i] = memoized_lazy_function(
                        static, url
                    )  # lazy variant of `static(url)`
            ...
            if plugin_dict.get("auto-include", False):
                PLUGINS[PLUGINS_DEFAULT].setdefault(
                    resource_type, ListWithLazyItems()
                ).extend(urls)

            # also append it to the ALL pseudo-plugin;
            PLUGINS[PLUGIN_ALL].setdefault(resource_type, ListWithLazyItems()).extend(
                urls
            )
