...
class Pg:
    ...
    def get_columns_names(self, table, schema_name='public'):
        columns = []
        schema = 'public'
        try:
            ...
            with conn.cursor() as col_cursor:
                col_names_str = f"SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema={schema_name} AND table_name={table}"
