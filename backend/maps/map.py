...
def my_map(land_parcels, parcel=None, lat=None, lng=None):
    ...
    map5 = raster_layers.TileLayer(tiles="Stamen Watercolor", name="Watercolor").add_to(
        m
    )
    ...
    folium.plugins.Search(
        ...
        collapsed=False,
        search_zoom=18,
        search_label="lrnumber",
        weight=3,
    ).add_to(m)
    if parcel:
        folium.features.GeoJson(
            parcel,
            style_function=lambda x: style_parcel,
            highlight_function=None,
            name="Parcels",
            overlay=True,
            control=True,
    ...
    img = folium.features.DivIcon(
        html="<div> <img src='img.jpg' width='500' height='600'></div>",
        icon_size=None,
        icon_anchor=None,
        popup_anchor=None,
        class_name="empty",
    ...
    folium.LayerControl(
        position="topright", strings={"title": "my location", "popup": "Your position"}
    ).add_to(m)
